...
class TrackStorage:
    ...
    def update(self, 
               track_pool: np.ndarray, dets: np.ndarray, 
               frame_ids: int | np.ndarray,
               embs_track_pool: np.ndarray = None, embs: np.ndarray = None):
        ...
        if embs is not None:
            self.embs[embs_track_pool] = EmbeddingHandler.update(
                self.embs[embs_track_pool], embs)
