...
class PureTrackNew(BaseTracker):
    ...
    def __init__(
        ...
        max_emb_iou_overlap_thresh: float = 0.65
    ):
        ...
        self.max_emb_iou_overlap_thresh = max_emb_iou_overlap_thresh
    ...
    def get_overlapped_dets(self, dets: np.ndarray):
        ious = iou_batch_xywh(dets[:, :4], dets[:, :4])

        res = np.where((ious > self.max_emb_iou_overlap_thresh) &
                       (ious < 0.99))[0]
    ...
    def process_matches(self,
        ...
        if embs is not None:
            update_embs_pool = np.intersect1d(tracks_embs_pool,
                                              update_tracks_pool)
            reactivate_embs_pool = np.intersect1d(tracks_embs_pool,
                                                  reactivate_tracks_pool)

            update_embs = embs[np.in1d(tracks_embs_pool,
                                       update_tracks_pool)]
            reactivate_embs = embs[np.in1d(tracks_embs_pool,
                                           reactivate_tracks_pool)]
        else:
            update_embs_pool = None
            reactivate_embs_pool = None
            update_embs = None
    ...
    def update(self,
        ...
        dets_overlapped_pool = self.get_overlapped_dets(dets_high)
        ...
        # Filter out embeddings update for too overlapped detections
        embs_update_filter = ~np.in1d(dets_matches_pool, dets_overlapped_pool)
        tracks_embs_update_pool = tracks_matches_pool[embs_update_filter]
        dets_embs_update_pool = dets_matches_pool[embs_update_filter]
        ...
        high_activated_pool, high_reactivated_pool = self.process_matches(
            ...
            tracks_embs_update_pool, embs_high[dets_embs_update_pool])
        ...
        self.tracks_storage.update(tracks_matches_pool,
                                   ...
                                   embs_high[dets_embs_update_pool])
