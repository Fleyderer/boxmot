...
from boxmot.trackers.puretracker.kalman.xywh_kf import KalmanFilterXYWH
...
class TrackStorage:
    means: np.ndarray[float] = TrackStorageProperty('means') # xcycwh
    ...
    pure_embs: np.ndarray[float] = TrackStorageProperty('pure_embs')
    history: dict[str, list[np.ndarray]] = {} # xcycwh
    ...
    def translate_track_ids(self,
        ...
        if isinstance(track_ids, (int, float)):
            track_ids = [track_ids]
    ...
    def from_dets(self, tracks_pool: np.ndarray,
        ...
        if embs is not None:
            ...
            self.pure_embs[tracks_pool] = embs

    def update(self, track_pool: np.ndarray,
               dets: np.ndarray, frame_ids: int | np.ndarray,
               embs: np.ndarray = None,
               pure_track_pool: np.ndarray = None,
               pure_embs: np.ndarray = None):
        ...
        if pure_track_pool is not None and pure_embs is not None:
            self.pure_embs[pure_track_pool] = EmbeddingHandler.update(
                self.pure_embs[pure_track_pool], pure_embs)
        ...
        for track_id in track_pool:
            ...
            self.history[track_id].append(self.means[track_id][:, :4][0])
    ...
    def reactivate(self, track_pool: np.ndarray,
        ...
        # TODO: Check is this a good way to update pure embs
        self.pure_embs[track_pool] = self.embs[track_pool]
        self.confs[track_pool] = dets[:, 4]
        self.classes[track_pool] = dets[:, 5]
        self.det_ids[track_pool] = dets[:, 6]
